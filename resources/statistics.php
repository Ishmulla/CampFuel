...
// $s7=mysqli_query($con,"SELECT * FROM tbl_orders WHERE client_id='$user_id' AND order_status='4' AND date_posted BETWEEN '$from_timestamp' AND '$to_timestamp'");
// $cancelled=mysqli_num_rows($s6);
...
<!-- TABS -->
<div class="tab">
  <button id="fst" class="tablinks" onclick="openCity(event, 'Paris')">Total Expenditure</button>
  <button class="tablinks" onclick="openCity(event, 'Tokyo')">Best seller</button>
...
<!-- Expenditure -->
<?php
$sqlexpenditure=mysqli_query($con,"SELECT SUM(total_price) AS total_price FROM tbl_orders WHERE client_id='$user_id' AND date_posted BETWEEN '$from_timestamp' AND '$to_timestamp' AND order_status='4'");
if (mysqli_num_rows($sqlexpenditure)>0) {
while ($rowex=$sqlexpenditure->fetch_assoc()) {
$general_expenses=$rowex['total_price'];
}}
?>
<div class="notification "><p><b>Total expenditure:</b> Ksh <?php echo $general_expenses; ?></p></div> 
<!-- Expenditure -->
</div>
...
<?php
$best_seller=mysqli_query($con,"SELECT merchant_id, COUNT(merchant_id) AS bs 
  FROM tbl_orders
  WHERE client_id='$user_id' AND date_posted BETWEEN '$from_timestamp' AND '$to_timestamp'
  GROUP BY merchant_id
  ORDER BY bs DESC
  LIMIT 1");
if (mysqli_num_rows($best_seller)>0) {
while ($rowbs=$best_seller->fetch_assoc()) {
$bsid=$rowbs['merchant_id'];
  $name=mysqli_query($con,"SELECT * FROM tbl_companies WHERE merchant_id='$bsid'");
...
?>
<div class="notification">
  <p>There is no best seller yet.</p>
</div>
<?php
}
?>
...
<!-- General -->
<table id="content" class="table is-stipped is-fullwidth notification">
<tbody>
<tr>
  <td>
...
<tr>
  <td>
<p>Total orders made : <?php echo $total; ?></p>     
  </td>
</tr>
<tr>
  <td>
<p>Orders awaiting confirmation : <?php echo $pending; ?></p>    
  </td>
</tr>
<tr>
  <td>
<p>Orders under shipping : <?php echo $shipping; ?></p>    
  </td>
</tr>
<tr>
  <td>
<p>Orders awaiting payment : <?php echo $awaiting_payment; ?></p>    
  </td>
</tr>
<tr>
  <td>
<p>Cancelled orders : <?php echo $cancelled; ?></p>    
  </td>
</tr>
<tr>
  <td>
<p>Completed orders : <?php echo $completed; ?></p>
  </td>
</tr>
<tr>
  <td>
    <b>TOTAL EXPENDITURE : </b>Ksh <?php echo $general_expenses; ?>
  </td>
</tr>
<tr>
  <td>
...
function header(doc)
{
    ...
    doc.text("CAMPFUEL REPORT", margins.left + 50, 40 );
  doc.setLineCap(2);
